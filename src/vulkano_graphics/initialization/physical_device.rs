...
pub fn get_physical_device<'a>(
  instance: &'a Arc<Instance>, // physical device should not outlive the instance
  g_device_extensions: &DeviceExtensions,
  surface: &Arc<Surface<Window>>,
) -> PhysicalDevice<'a> {
  let device = PhysicalDevice::enumerate(instance)
    .filter(|&p| p.supported_extensions().is_superset_of(g_device_extensions))
    .filter(|p| {
      p.queue_families()
